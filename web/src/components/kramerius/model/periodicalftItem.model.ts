export class PeriodicalFtItem {
	...
	getPath(): string {
		if (this.type === 'page' || this.type === 'monograph_unit_page') {
			const uuid =
				...
				this.context['monographunit'] ||
				this.context['periodicalitem'] ||
				this.context['supplement'] ||
				this.context['periodicalvolume'];
			return 'view/' + uuid;
			...
			const uuid =
				this.context['periodicalitem'] || this.context['periodicalvolume'];
			return 'view/' + uuid;
		} else {
	...
	getQuery() {
		if (
			this.type === 'page' ||
			this.type === 'monograph_unit_page' ||
			this.type === 'omnibus_unit_page'
		) {
